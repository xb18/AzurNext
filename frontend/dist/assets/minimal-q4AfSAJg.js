...
:root {
  ...
  --theme-radius-card: 14px; --theme-radius-popover: 10px; --theme-radius-pill: 999px;
  ...
  --theme-nav-popover-bg: var(--surface); --theme-nav-popover-edge: var(--border); --theme-nav-popover-shadow: none;
  --theme-primary-bg: var(--accent); --theme-primary-hover: var(--accent-hover);
  --theme-input-bg: var(--surface); --theme-input-text: var(--text); --theme-input-border: var(--border);
  ...
  --theme-segment-bg: var(--surface-muted); --theme-segment-border: var(--border); --theme-segment-indicator: var(--accent-soft);
...
/* 浮层与控件补充：这些不在上面的骨架 / 卡片清单里，是圆角审计逐页扫出来的漏网项
   （下拉浮层、指标芯片、复选框、开关、标签、logo 等）。新增控件后可用同样的扫描方式复查。 */
...
:root[data-theme='extreme'] .toggle,
:root[data-theme='extreme'] .toggle > span,
...
/* overview-page 原本用 ::after 预留约 5% 底部占位，这里把空间还给日志面板 */
...
:root[data-theme='extreme'] .config-toolbar {margin-bottom: 10px}
:root[data-theme='extreme'] .config-group {margin-bottom: 9px}
...
:root[data-theme='extreme'] .summary-metrics-panel {padding: 10px 14px}
:root[data-theme='extreme'] .summary-metrics-panel .summary-metrics {gap: 8px 12px; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr))}
...
/* 面包屑里的实例切换按钮原本是侧栏那种大块选择器，收成与面包屑文字对齐的单行小按钮 */
...
.toggle::before {content: ''; position: absolute}
