...
5. **提交数量控制与聚合门限 (Squash Gate)**：私有提交数量建议常年保持在 5~10 个。当 `master..main` 的私有提交数达到或超过 **20 个** 时，必须进行模块级聚合（Squash），按功能领域整合成高内聚的原子提交，杜绝长期维护中的冲突乘数效应。
...
### 第六步：提交数量检查与智能聚合门限 (Squash Gate)
在变基成功且测试通过后，检查当前分支私有提交总数：
```powershell
git rev-list --count master..main
```
- **提交数 < 20**：无需聚合，直接进行远程推送。
- **提交数 >= 20**：**触发智能聚合整理流程**。将零散的微调提交按功能领域归类压缩为 5~8 个原子性的高质量提交（推荐 7 大模块分类）：
  1. `feat(desktop)`: 桌面启动器外壳接入、Thin Shell 协议与多步更新交互
  2. `feat(scheduler)`: 多配置实例全局调度中心与状态看板
  3. `feat(webui)`: 默认网络/免密策略调整、路由优化与 AzurNext 品牌标识
  4. `feat(deploy)`: 启动脚本、ADB 多源下载、端口自适应分配与更新策略
  5. `feat(login)`: 4399 渠道服活动弹窗自动关闭与悬浮球配置联动
  6. `feat(game)`: 游戏自动化业务逻辑增强（补给/共斗/委托/退役/奖励防卡等）
  7. `docs(agent)`: 规范项目开发准则，新增相关工具与技能规范
- 聚合后验证最终 diff（`git diff backup-main..main`）确保代码零丢失。
